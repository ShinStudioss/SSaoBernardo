...
function Crystal_Shadow(_owner, _shadowType, _transformMode=SHADOW_SOFT_TRANSFORMED) constructor {
	...
	static Destroy = function() {
		...
		if (__type == CRYSTAL_SHADOW.STATIC && __renderer != undefined) {
			__renderer.__vertexShadowsStaticRebuild = true;
		}
	}
	...
	/// @desc Add meshes based on the shape of a sprite. If the sprite has multiple frames (subimgs), each frame will be a shadow mesh.
	/// WARNING: Doesn't work well with concave or square shapes.
	/// @method AddMeshesFromSprite(sprite, maxPoints)
	static AddMeshesFromSprite = function(_sprite, _maxPoints=32) {
		var _frames = sprite_get_number(_sprite);
		if (_frames == 1) {
			__frames[0] = new Crystal_ShadowMesh().FromSpriteConvexHull(_sprite, 0, _maxPoints);
			frameCount = 1;
		} else {
			var i = 0;
			repeat(_frames) {
				__frames[i] = new Crystal_ShadowMesh().FromSpriteConvexHull(_sprite, i, _maxPoints);
				++i
			}
			frameCount = i;
		}
...
function Crystal_ShadowMesh() constructor {
	...
	/// @param {Real} xOffset x offset of all points. Default is 0.
	/// @param {Real} yOffset y offset of all points. Default is 0.
	static FromArray = function(_array, _xOffset=0, _yOffset=0) {
		var _points = _array,
		...
		while(i < _len) {
			_nx = _points[i++];
			_ny = _points[i++];
			AddEdge(_px-_xOffset, _py-_yOffset, _nx-_xOffset, _ny-_yOffset);
			_px = _nx;
			_py = _ny;
		}
		return self;
	...
	/// @desc Creates a polygon based on the sprite convex hull (sprite tracing). The rotation and scale are from the Crystal_Shadow() itself.
	/// @method FromSpriteConvexHull(sprite, maxPoints)
	/// @param {Asset.GMSprite} sprite The sprite asset to generate vertex points from.
	/// @param {Real} subimg The sprite subimg (frame).
	/// @param {Real} maxPoints The maximum number of points allowed. 32 is a good number for performance reasons.
	static FromSpriteConvexHull = function(_sprite, _subimg=0, _maxPoints=32) {
		FromArray(sprite_get_convex_hull(_sprite, _maxPoints, _subimg), sprite_get_xoffset(_sprite), sprite_get_yoffset(_sprite));
		return self;
	}
	
	/// @desc Creates a polygon based on a previously created convex hull (sprite tracing using sprite_get_convex_hull()). The rotation and scale are from the Crystal_Shadow() itself.
	/// @method FromConvexHull(convexHull)
	/// @param {Array} convexHull The array with points created with sprite_get_convex_hull().
	static FromConvexHull = function(_convexHull) {
		FromArray(_convexHull);
		return self;
	}
