...
function __Crystal_UISystem() constructor {
	...
	static RemoveContainer = function(_container, _freeMemory=true) {
		...
		for (var i = 0; i < _size; ++i) {
			if (__containers[i] == _container) {
				...
				if (_freeMemory && _container.Destroy != undefined) _container.Destroy();
	...
	/// @desc Removes all containers from the UI
	static RemoveContainers = function(_freeMemory=false) {
		if (_freeMemory) {
			for (var i = 0; i < array_length(__containers); ++i) {
				// call Destroy() function from container
				// the container MUST be responsible for destroying its children and freeing memory (like surfaces)
				var _container = __containers[i];
				if (_container.Destroy != undefined) _container.Destroy();
			}
		}
		// remove containers from list
		array_resize(__containers, 0);
	}
