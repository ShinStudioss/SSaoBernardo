...
function __Crystal_DebugInterfaces() constructor {
	...
	static PanelClassCrystalRenderer = function() {
		...
		var _lightsMenu = _inspector.AddElement(new __Crystal_UIElementSection("Lights", false, 2), _menuA);
			...
			_inspector.AddElement(new __Crystal_UIElementCheckbox(_ref, "__ambientLightColorIsOverlay", "Ambient Color is Overlay", "SetAmbientColorOverlayEnable", [-1], 0), _lightsMenu);
			_inspector.AddElement(new __Crystal_UIElementText("Ambient LUT:"), _lightsMenu);
			_inspector.AddElement(new __Crystal_UIElementCustom(function() {
				LUTTempSprite = -1;
				oldLUTTexture = -1;
			}, function(_element, _ui) {
				var _texture = __panelCurrentClass.__ambientLutTex;
				if (_texture != undefined) {
					var _size = 24;
					_element.height = _size;
					var _xx = _element.xx;
					var _yy = _element.yy;
					var _color = c_white;
					// load new LUT texture
					if (point_in_rectangle(_ui.__inputCursorX, _ui.__inputCursorY, _xx, _yy, _xx+_size, _yy+_size)) {
						if (_ui.__inputLeft) {
							_color = c_gray;
						}
						if (_ui.__inputLeftReleased) {
							oldLUTTexture = _texture;
							var _file = get_open_filename("PNG Images|*.png", "");
							if (_file != "") {
								if (sprite_exists(LUTTempSprite)) sprite_delete(LUTTempSprite);
								LUTTempSprite = sprite_add(_file, 0, false, false, 0, 0);
								var _type = 1, _horizontalSquares = 8;
								// assume that it's a strip LUT
								if (sprite_get_width(LUTTempSprite) > sprite_get_height(LUTTempSprite)) {
									_type = 0;
									_horizontalSquares = 16;
								}
								__panelCurrentClass.SetAmbientLUT(sprite_get_texture(LUTTempSprite, 0), _type, _horizontalSquares);
							}
						}
						if (_ui.__inputMiddleReleased) {
							if (oldLUTTexture != -1) {
								__panelCurrentClass.SetAmbientLUT(oldLUTTexture);
							}
						}
						if (_ui.__inputRightReleased) {
							__panelCurrentClass.SetAmbientLUT(sprite_get_texture(__cle_sprNeutralLUT, 0));
						}
					}
					// draw quad
					...
					// texture reference
					draw_text(_xx+_size+8, _yy, _texture);
				}
			}), _lightsMenu);
			_inspector.AddElement(new __Crystal_UIElementSeparator(), _lightsMenu);
		...
		// > Shadows
		var _shadowsMenu = _inspector.AddElement(new __Crystal_UIElementSection("Shadows", false, 2), _menuA);
			_inspector.AddElement(new __Crystal_UIElementCheckbox(_ref, "__isShadowsEnabled", "Shadows Enable", "SetShadowsEnable", [-1], 0), _shadowsMenu);
			// Vertex Shadows
			var _vertexShadowsMenu = _inspector.AddElement(new __Crystal_UIElementSection("Vertex Shadows", false, 2), _shadowsMenu);
				_inspector.AddElement(new __Crystal_UIElementCustom(, function(_element, _ui) {
					var _class = __panelCurrentClass;
				
					var _dynamicSize = ds_list_size(_class.__vertexShadowsDynamic);
					var _visibleDynamic = 0;
					var d = 0;
					repeat(_dynamicSize) {with(_class.__vertexShadowsDynamic[| d++]) if (!__cull) _visibleDynamic++;}
				
					var _staticSize = ds_list_size(_class.__vertexShadowsStatic);
					var _visibleStatic = 0;
					var d = 0;
					repeat(_staticSize) {with(_class.__vertexShadowsStatic[| d++]) if (!__cull) _visibleStatic++;}
				...
				_inspector.AddElement(new __Crystal_UIElementButton("Update Static Shadows", function() {__panelCurrentClass.__vertexShadowsStaticRebuild = true;}), _vertexShadowsMenu);
				// EDITOR
				...
				_inspector.AddElement(new __Crystal_UIElementCustom(function(_element) {
					...
					ds_list_add(__overlayRenderables, function() {
						// visualize shadows
						if (__panelCurrentClass.__isShadowsEnabled) {
							draw_set_color(c_white);
							shader_set(__cle_shDebugShadow);
							shader_set_uniform_f(debugShadowsShaderUniformColor, 1, 0, 1);
							if (showStaticShadowsOverlay) vertex_submit(__panelCurrentClass.__vBuffVertexShadowsStatic, pr_linelist, -1);
							shader_set_uniform_f(debugShadowsShaderUniformColor, 0, 1, 1);
							if (showDynamicShadowsOverlay) vertex_submit(__panelCurrentClass.__vBuffVertexShadowsDynamic, pr_linelist, -1);
							shader_reset();
						}
					});
				}), _vertexShadowsMenu);
			...
			// Sprite Shadows
			var _spriteShadowsMenu = _inspector.AddElement(new __Crystal_UIElementSection("Sprite Shadows", false, 2), _shadowsMenu);
			_inspector.AddElement(new __Crystal_UIElementCustom(, function(_element, _ui) {
					var _class = __panelCurrentClass;
				
					var _dynamicSize = ds_list_size(_class.__spriteShadowsDynamic);
					var _visibleDynamic = 0;
					var d = 0;
					repeat(_dynamicSize) {with(_class.__spriteShadowsDynamic[| d++]) if (!__cull) _visibleDynamic++;}
				
					var _staticSize = ds_list_size(_class.__spriteShadowsStatic);
					var _visibleStatic = 0;
					var d = 0;
					repeat(_staticSize) {with(_class.__spriteShadowsStatic[| d++]) if (!__cull) _visibleStatic++;}
				...
				_inspector.AddElement(new __Crystal_UIElementText("(visible/total)"), _spriteShadowsMenu);
				_inspector.AddElement(new __Crystal_UIElementButton("Update Static Shadows", function() {__panelCurrentClass.__spriteShadowsStaticRebuild = true;}), _spriteShadowsMenu);
				
				// EDITOR
				_inspector.AddElement(new __Crystal_UIElementText("Shadows:"), _spriteShadowsMenu);
				_inspector.AddElement(new __Crystal_UIElementCustom(function(_element) {
					_element.alwaysVisible = true;
					debugShadowsShaderUniformColor = shader_get_uniform(__cle_shDebugShadow, "u_color");
					showStaticSpriteShadowsOverlay = false;
					showDynamicSpriteShadowsOverlay = false;
				}, function(_element, _ui) {
					_element.height = 0;
					__panelInspector.__bake(); // make sure inspector continues drawing every frame
					ds_list_add(__overlayRenderables, function() {
						// visualize shadows
						draw_set_color(c_white);
						shader_set(__cle_shDebugSpriteShadow);
						shader_set_uniform_f(debugShadowsShaderUniformColor, 1, 0, 1);
						if (showStaticSpriteShadowsOverlay) {
							var _array = __panelCurrentClass.__vBuffSpriteShadowsStaticBatches;
							for (var v = 0; v < array_length(_array); v+=2) {
								vertex_submit(_array[v], pr_trianglelist, _array[v+1]);
							}
						}
						shader_set_uniform_f(debugShadowsShaderUniformColor, 0, 1, 1);
						if (showDynamicSpriteShadowsOverlay) {
							var _array = __panelCurrentClass.__vBuffSpriteShadowsDynamicBatches;
							for (var v = 0; v < array_length(_array); v+=2) {
								vertex_submit(_array[v], pr_trianglelist, _array[v+1]);
							}
						}
				...
				_inspector.AddElement(new __Crystal_UIElementCheckbox(self, "showStaticSpriteShadowsOverlay", "Show Static Shadows"), _spriteShadowsMenu);
				_inspector.AddElement(new __Crystal_UIElementCheckbox(self, "showDynamicSpriteShadowsOverlay", "Show Dynamic Shadows"), _spriteShadowsMenu);
	...
	static PanelClassMaterialLayer = function() {
		...
		_inspector.AddElement(new __Crystal_UIElementSlider(_ref, "emission",, 0, 10), _menuA);
		
		_inspector.AddElement(new __Crystal_UIElementText("Layer Range List:"), _menuA);
		
		var i = 0, isize = array_length(_ref.__ranges);
		repeat(isize) {
			var _lay = _ref.__ranges[i];
	...
	static InspectorPointLight = function() {
		...
		var _shaderMenu = _inspector.AddElement(new __Crystal_UIElementSection("Shader", false, 2), _menuA);
			...
			_inspector.AddElement(new __Crystal_UIElementRadio(__editorSelectedInstance, "shaderType", ["Basic", "Phong", "BRDF"], [LIGHT_SHADER_BASIC, LIGHT_SHADER_PHONG, LIGHT_SHADER_BRDF]), _shaderMenu);
			_inspector.AddElement(new __Crystal_UIElementText("shadowShaderType"), _shaderMenu);
			_inspector.AddElement(new __Crystal_UIElementRadio(__editorSelectedInstance, "shadowShaderType", ["Basic", "Smooth", "Sprite", "Sprite + Basic", "Sprite + Smooth"], [LIGHT_SHADOW_SHADER_BASIC, LIGHT_SHADOW_SHADER_SMOOTH, LIGHT_SHADOW_SHADER_SPRITE, LIGHT_SHADOW_SHADER_SPRITE_BASIC, LIGHT_SHADOW_SHADER_SPRITE_SMOOTH]), _shaderMenu);
		var _colorMenu = _inspector.AddElement(new __Crystal_UIElementSection("Color", false, 2), _menuA);
		...
		_inspector.AddElement(new __Crystal_UIElementSlider(__editorSelectedInstance, "shadowAlpha",, 0, 1), _menuA);
	...
	static InspectorSpotLight = function() {
		...
		_inspector.AddElement(new __Crystal_UIElementSlider(__editorSelectedInstance, "spotFOV",, 0, 180, 1), _menuA);
		...
		_inspector.AddElement(new __Crystal_UIElementSlider(__editorSelectedInstance, "cookieAlpha",, 0, 1), _menuA);
		_inspector.AddElement(new __Crystal_UIElementCheckbox(__editorSelectedInstance, "castShadows"), _menuA);
		_inspector.AddElement(new __Crystal_UIElementCheckbox(__editorSelectedInstance, "selfShadows"), _menuA);
		_inspector.AddElement(new __Crystal_UIElementSlider(__editorSelectedInstance, "penetration",, 0, 5), _menuA);
		_inspector.AddElement(new __Crystal_UIElementSlider(__editorSelectedInstance, "shadowPenumbra",, 0, 50), _menuA);
...
function Crystal_DebugUI(_origin=undefined, _classInstance=undefined, _isOpened=false, _startMaximized=true) constructor {
	...
	static Draw = function(_font=__cle_fntDebugUI) {
		...
		if (_overlayRenderablesSize > 0) {
			...
			matrix_set(matrix_projection, camera_get_proj_mat(_cam));
			matrix_set(matrix_view, camera_get_view_mat(_cam));
				...
				if (__editorIsEditingLights) {
					...
					if (__uiSystem.__containerInFocus == undefined) {
						...
						if (__editorLightEditing) {
							...
							// Clone
							if (__uiSystem.__inputMiddlePressed) {
								if (__editorSelectedInstance != noone) {
									with(__editorSelectedInstance) {
										var _copyInst = instance_copy(false);
										_copyInst.x += 32;
									}
									__inspector.__bake();
								}
