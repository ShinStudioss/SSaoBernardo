...
/// You may also need two post-processing renderer, one for each lighting renderer.
function Crystal_Renderer() constructor {
	// Copyright (C) 2025, Mozart Junior (FoxyOfJungle)
	__crystal_trace("Renderer created", 3);
	// Init
	__renderSurfaceWidth = 0;
	__renderSurfaceHeight = 0;
	__oldRenderSurfaceWidth = 0;
	__oldRenderSurfaceHeight = 0;
	__sourceSurface = -1;
	__sourceCamera = -1;
	...
	__ambientLightColorIsOverlay = false;
	...
	__surfaceCamera = -1;
	__viewProjMatrix = matrix_build_identity();
	...
	// Shadows-related
	__vertexFormatVertexShadows = __crystalGlobal.vformatVertexShadows;
	__vBuffVertexShadowsStatic = undefined;
    __vBuffVertexShadowsDynamic = undefined;
	__vertexShadowsStaticRebuild = false;
	__vertexShadowsStatic = ds_list_create();
	__spriteShadowsStatic = ds_list_create();
	__vertexFormatSpriteShadows = __crystalGlobal.vformatVertexSpriteShadows;
	__vBuffSpriteShadowsStaticBatches = [];
	__vBuffSpriteShadowsDynamicBatches = [];
	__spriteShadowsStaticRebuild = false;
	__vertexShadowsDynamic = ds_list_create();
	__spriteShadowsDynamic = ds_list_create();
	...
	/// @ignore
	static __cleanShadowBuffers = function() {
		__vertexShadowsStaticRebuild = true;
		__spriteShadowsStaticRebuild = true;
		if (__vBuffVertexShadowsStatic != undefined) {
			vertex_delete_buffer(__vBuffVertexShadowsStatic);
			__vBuffVertexShadowsStatic = undefined;
		}
		if (__vBuffVertexShadowsDynamic != undefined) {
			vertex_delete_buffer(__vBuffVertexShadowsDynamic);
			__vBuffVertexShadowsDynamic = undefined;
		}
		for (var v = 0; v < array_length(__vBuffSpriteShadowsStaticBatches); v+=2) {
			vertex_delete_buffer(__vBuffSpriteShadowsStaticBatches[v]);
		}
		array_resize(__vBuffSpriteShadowsStaticBatches, 0);
		for (var v = 0; v < array_length(__vBuffSpriteShadowsDynamicBatches); v+=2) {
			vertex_delete_buffer(__vBuffSpriteShadowsDynamicBatches[v]);
		}
		array_resize(__vBuffSpriteShadowsDynamicBatches, 0);
	}
	
	/// @ignore
	static __addShadowCaster = function(_shadow) {
		switch(_shadow.__type) {
			case CRYSTAL_SHADOW.STATIC: ds_list_insert(__vertexShadowsStatic, 0, _shadow); __vertexShadowsStaticRebuild = true; break;
			case CRYSTAL_SHADOW.DYNAMIC: ds_list_insert(__vertexShadowsDynamic, 0, _shadow); break;
			default: __crystal_trace($"Vertex ShadowCaster failed to be added. Unknown type {_shadow.__type}", 1); exit; break;
		}
	}
	
	/// @ignore
	static __addSpriteShadowCaster = function(_shadow) {
		switch(_shadow.__type) {
			case CRYSTAL_SHADOW.STATIC: ds_list_insert(__spriteShadowsStatic, 0, _shadow); __spriteShadowsStaticRebuild = true; break;
			case CRYSTAL_SHADOW.DYNAMIC: ds_list_insert(__spriteShadowsDynamic, 0, _shadow); break;
			default: __crystal_trace($"Sprite ShadowCaster failed to be added. Unknown type {_shadow.__type}", 1); exit; break;
		}
	}
	...
	static __calculateVRAM = function() {
		...
		// material layers
		var _materialLayers = [__matNormalLayers, __matMaterialLayers, __matReflectionLayers, __matEmissiveLayers, __matLightLayers, __matCombineLayers], _matLayer = undefined;
		var m = 0, msize = array_length(_materialLayers);
		repeat(msize) {
		...
		// Vertex buffers
		var _vertexBuffers = [__vBuffVertexShadowsStatic, __vBuffVertexShadowsDynamic], _vBuff = undefined; // TO-DO: add sprite shadows
		for (var i = 0; i < array_length(__vBuffSpriteShadowsStaticBatches); i+=2) {array_push(_vertexBuffers, __vBuffSpriteShadowsStaticBatches[i]);}
		for (var i = 0; i < array_length(__vBuffSpriteShadowsDynamicBatches); i+=2) {array_push(_vertexBuffers, __vBuffSpriteShadowsDynamicBatches[i]);}
		i = 0; isize = array_length(_vertexBuffers);
		repeat(isize) {
			_vBuff = _vertexBuffers[i];
			if (_vBuff != undefined) {
				__gpuVRAMusage += vertex_get_buffer_size(_vBuff);
			}
			++i;
		}
	...
	#region GENERAL
	/// @desc Destroy the Renderer from memory. Including shadows vertex buffer, collision buffer and pass surfaces.
	/// This should only be called when you are 100% sure that you will no longer use the same renderer instance.
	/// @method Destroy()
	static Destroy = function() {
		if (__destroyed) exit;
		if (__vBuffVertexShadowsStatic != undefined) vertex_delete_buffer(__vBuffVertexShadowsStatic);
		if (__vBuffVertexShadowsDynamic != undefined) vertex_delete_buffer(__vBuffVertexShadowsDynamic);
		...
		__cleanSurfaces();
		ds_list_destroy(__vertexShadowsStatic);
		ds_list_destroy(__vertexShadowsDynamic);
		...
		__crystal_trace("Lighting renderer destroyed", 3);
		__destroyed = true;
	}
	#endregion
	...
	/// @desc Enable or disable vertex and sprite shadows generation, even if you have created shadows. Disabling and then re-enabling this setting will cause the shadows to regenerate.
	/// This feature can be useful for your game performance options.
	/// @method SetShadowsEnable(enabled)
	/// @param {Bool} enabled Defines if the renderer is generating shadows from Crystal. Use -1 to toggle.
	static SetShadowsEnable = function(_enabled=-1) {
		if (_enabled == -1) {
			__isShadowsEnabled = !__isShadowsEnabled;
		} else {
			__isShadowsEnabled = _enabled;
		}
		__cleanShadowBuffers();
		return self;
	}
	...
	/// @desc Defines whether the ambient light color is an ovarlay (true) or not (false: multiplicative, which is the default behaviour).
	/// @method SetAmbientColorOverlayEnable(enabled)
	/// @param {Bool} enabled Defines if the lightmap is generating collisiong data. Use -1 to toggle.
	static SetAmbientColorOverlayEnable = function(_enabled=-1) {
		if (_enabled == -1) {
			__ambientLightColorIsOverlay = !__ambientLightColorIsOverlay;
		} else {
			__ambientLightColorIsOverlay = _enabled;
		}
		return self;
	}
	...
	static SetRenderResolution = function(_resolution=1) {
		...
		__oldRenderSurfaceWidth = 0; // reset old resolution, so we can simulate a resize
		__oldRenderSurfaceHeight = 0;
		return self;
	}
	...
	/// NOTE: You need to enable light collisions with .SetLightsCollisionEnable() for this to work. This will copy the lightmap surface to a buffer.
	...
	static GetLightsCollisionAt = function(_xPosition, _yPosition) {
		...
		var _NDC = matrix_transform_vertex(__viewProjMatrix, _xPosition, _yPosition, 0);
		...
		var _position = (_x + _y * __lightmapSurfaceWidth) * __lightmapBytesPerPixel;
		var _r = 0, _g = 0, _b = 0, _a = 0;
		// 8 bits
		if (__lightmapBytesPerPixel == 4) {
	...
	/// @desc Renderize lights, shadows, materials and other things on the internal surface, which is a copy from the input surface. Also returns the final surface.
	/// It must be executed in the Draw End event.
	/// It uses the size of the referenced surface for internal rendering resolution (example: application_surface size).
	/// WARNING! The renderer will use the depth buffer of the input surface, so keep that in mind. If the input surface does not have a depth buffer, you will not be able to perform sorting of lights, shadows and materials.
	...
	static Render = function(_surface, _camera) {
		...
		if (!surface_exists(_surface)) {
			if (__allowRendering) {
				...
				__crystal_trace("WARNING: trying to renderize using non-existent surface (or zero size)", 2);
			}
			exit;
		}
		...
		if (__isRenderEnabled) {
			...
			if (_inputSurfaceWidth != __oldRenderSurfaceWidth || _inputSurfaceHeight != __oldRenderSurfaceHeight) {
				__cleanSurfaces();
				...
				__oldRenderSurfaceWidth = _inputSurfaceWidth;
				__oldRenderSurfaceHeight = _inputSurfaceHeight;
			}
			...
			#region Culling
				if (__isCullingEnabled) {
					...
					if (point_distance(_camCenterX, _camCenterY, __cullingDynamicsOldCamX, __cullingDynamicsOldCamY) > __cullingDynamicsViewMoveDistance || _camW != __cullingDynamicsOldCamW || _camH != __cullingDynamicsOldCamH || __cullingDynamicsUpdateNow) {
						...
						var
						dssize = ds_list_size(__vertexShadowsDynamic),
						...
						if (CLE_CFG_CAMERA_ROTATION) {
							...
							var k = 0;
							repeat(dssize) {with(__vertexShadowsDynamic[| k++]) {__cull = point_distance(x, y, _camCenterX, _camCenterY) > _cullSize;}}
							k = 0;
							repeat(dssize) {with(__spriteShadowsDynamic[| k++]) {__cull = point_distance(x, y, _camCenterX, _camCenterY) > _cullSize;}}
							k = 0;
							...
							var k = 0;
							repeat(dssize) {with(__vertexShadowsDynamic[| k++]) {__cull = (( x < _cX1 || x > _cX2) || (y < _cY1 || y > _cY2));}}
							k = 0;
							repeat(dssize) {with(__spriteShadowsDynamic[| k++]) {__cull = (( x < _cX1 || x > _cX2) || (y < _cY1 || y > _cY2));}}
							k = 0;
					...
					if (point_distance(_camCenterX, _camCenterY, __cullingStaticsOldCamX, __cullingStaticsOldCamY) > __cullingStaticsViewMoveDistance) {
						...
						__vertexShadowsStaticRebuild = true; // enable static shadows update
					
						// disable stuff outside view
						var sssize = ds_list_size(__vertexShadowsStatic);
						// all static shadows
						if (CLE_CFG_CAMERA_ROTATION) {
							var _cullSize = min(_camW, _camH) + __cullingStaticsViewBorderSize / 2;
							var k = 0;
							repeat(sssize) {with(__vertexShadowsStatic[| k++]) {__cull = point_distance(x, y, _camCenterX, _camCenterY) > _cullSize;}}
							k = 0;
							repeat(sssize) {with(__spriteShadowsStatic[| k++]) {__cull = point_distance(x, y, _camCenterX, _camCenterY) > _cullSize;}}
						} else {
							var
							...
							var k = 0;
							repeat(sssize) {with(__vertexShadowsStatic[| k++]) {__cull = (( x < _cX1 || x > _cX2) || (y < _cY1 || y > _cY2));}}
							k = 0;
							repeat(sssize) {with(__spriteShadowsStatic[| k++]) {__cull = (( x < _cX1 || x > _cX2) || (y < _cY1 || y > _cY2));}}
						}
					}
				}
			#endregion
			...
			// Shadows generation
			#region Shadows Buffers
				if (__isShadowsEnabled) {
					#region >> Vertex Shadows
						var _vertexShadowFormat = __vertexFormatVertexShadows;
						
						#region Build Static Shadows
							var _vBuffVertexShadowsStatic = __vBuffVertexShadowsStatic;
							var _array = __vertexShadowsStatic;
							// rebuild static shadows if required
							if (__vertexShadowsStaticRebuild) {
								__vertexShadowsStaticRebuild = false;
								if (__vBuffVertexShadowsStatic != undefined) {
									vertex_delete_buffer(__vBuffVertexShadowsStatic);
									__vBuffVertexShadowsStatic = undefined;
							...
							if (__vBuffVertexShadowsStatic == undefined) {
								__vBuffVertexShadowsStatic = vertex_create_buffer();
								_vBuffVertexShadowsStatic  = __vBuffVertexShadowsStatic;
								vertex_begin(_vBuffVertexShadowsStatic, _vertexShadowFormat);
								// add vertices (shadows) to the buffer
								var isize = ds_list_size(_array); // using ds_list for performance...
								if (isize > 0) {
									var i = 0;
									repeat(isize) {
										with(_array[| i]) {
											...
											if (__destroyed) {
												...
												if (enabled && !__cull) {
													__mode(_vBuffVertexShadowsStatic);
												}
												++i;
											}
										}
								...
								vertex_end(_vBuffVertexShadowsStatic);
								// freeze vertex buffer to improve performance
								if (vertex_get_number(_vBuffVertexShadowsStatic) > 0) vertex_freeze(_vBuffVertexShadowsStatic);
							}
						#endregion
						
						#region Build Dynamic Shadows
							if (__vBuffVertexShadowsDynamic == undefined) {
								__vBuffVertexShadowsDynamic = vertex_create_buffer();
							}
							var _vBuffVertexShadowsDynamic = __vBuffVertexShadowsDynamic;
							var _array = __vertexShadowsDynamic;
							vertex_begin(_vBuffVertexShadowsDynamic, _vertexShadowFormat);
							// add vertices (shadows) to the buffer
							var isize = ds_list_size(_array);
							if (isize > 0) {
								var i = 0;
								repeat(isize) {
									with(_array[| i]) {
										...
										if (__destroyed) {
											...
											// add vertices from shadow points
											if (enabled && !__cull) {
												__mode(_vBuffVertexShadowsDynamic);
											}
											++i;
										}
									}
							...
							vertex_end(_vBuffVertexShadowsDynamic);
						#endregion
					#endregion
					
					#region >> Sprite Shadows
						var _spriteShadowFormat = __vertexFormatSpriteShadows;
						
						#region Build Static Shadows
							// array of vertex buffers
							var _vBuffSpriteShadowsStaticBatches = __vBuffSpriteShadowsStaticBatches;
							
							// rebuild static shadows if required
							if (__spriteShadowsStaticRebuild) {
								__spriteShadowsStaticRebuild = false;
								
								// build quads
								var _currentTexturePage = undefined;
								var _currentBatch = undefined;
								var _array = __spriteShadowsStatic;
								var isize = ds_list_size(_array);
								if (isize > 0) {
									// delete previously created vertex buffers
									for (var v = 0; v < array_length(_vBuffSpriteShadowsStaticBatches); v+=2) {
										vertex_delete_buffer(_vBuffSpriteShadowsStaticBatches[v]);
									}
									array_resize(_vBuffSpriteShadowsStaticBatches, 0);
									...
									repeat(isize) {
										with(_array[| i]) {
											...
											if (__destroyed) {
												...
												// add vertices from shadow points
												if (enabled && !__cull && sprite_exists(sprite)) {
													// get sprite texture page
													var _spriteInfo = sprite_get_info(sprite);
													var _texturePage = _spriteInfo.frames[frame % _spriteInfo.num_subimages].texture;
											
													// if different texture, create a new vertex buffer
													if (_currentTexturePage != _texturePage) {
														_currentTexturePage = _texturePage;
														if (_currentBatch != undefined) {
															vertex_end(_currentBatch); // finish previous vertex buffer
															if (vertex_get_number(_currentBatch) > 0) vertex_freeze(_currentBatch);
														}
														_currentBatch = vertex_create_buffer(); // create a new vertex buffer and push to draw
														array_push(_vBuffSpriteShadowsStaticBatches, _currentBatch, _currentTexturePage);
														vertex_begin(_currentBatch, _spriteShadowFormat);
													}
											
													__cle_shadowSpriteBuild(_currentBatch, _spriteInfo);
												}
												++i;
											}
										}
						...
						#region Build Dynamic Shadows
							// array of vertex buffers
							var _vBuffSpriteShadowsDynamicBatches = __vBuffSpriteShadowsDynamicBatches;
							
							// build quads
							var _currentTexturePage = undefined;
							var _currentBatch = undefined;
							var _array = __spriteShadowsDynamic;
							var isize = ds_list_size(_array);
							if (isize > 0) {
								// delete previously created vertex buffers
								for (var v = 0; v < array_length(_vBuffSpriteShadowsDynamicBatches); v+=2) {
									vertex_delete_buffer(_vBuffSpriteShadowsDynamicBatches[v]);
								}
								array_resize(_vBuffSpriteShadowsDynamicBatches, 0);
								...
								repeat(isize) {
									with(_array[| i]) {
										...
										if (__destroyed) {
											...
											// add vertices from shadow structs
											if (enabled && !__cull && sprite_exists(sprite)) {
												// get sprite texture page
												var _spriteInfo = sprite_get_info(sprite);
												var _texturePage = _spriteInfo.frames[frame % _spriteInfo.num_subimages].texture;
												...
												if (_currentTexturePage != _texturePage) {
													...
													_currentBatch = vertex_create_buffer(); // create a new vertex buffer and push to draw
													array_push(_vBuffSpriteShadowsDynamicBatches, _currentBatch, _currentTexturePage);
													vertex_begin(_currentBatch, _spriteShadowFormat);
												}
						...
						/// @ignore
						static __submitSpriteShadows = function(_staticBatchesArray, _dynamicBatchesArray) {
							for (var v = 0; v < array_length(_staticBatchesArray); v+=2) {
								vertex_submit(_staticBatchesArray[v], pr_trianglelist, _staticBatchesArray[v+1]);
							}
							for (var v = 0; v < array_length(_dynamicBatchesArray); v+=2) {
								vertex_submit(_dynamicBatchesArray[v], pr_trianglelist, _dynamicBatchesArray[v+1]);
							}
						}
					#endregion
				}
			...
			#region Render Lights and Shadows
				...
				if (__lightsIntensity > 0) {
					...
					surface_set_target(_pass.surface, _surface);
						...
						var _isShadowsEnabled = __isShadowsEnabled;
						
						#region Shared Uniforms + Blendmodes
							// Shadows
							static _u_shadowBasicParams = shader_get_uniform(__cle_shVertShadowBasic, "u_params"); // shadowScattering, shadowAlpha
							static _u_shadowBasicParams2 = shader_get_uniform(__cle_shVertShadowBasic, "u_params2"); // 2d light position, shadowDepthOffset
							static _u_shadowSmoothParams = shader_get_uniform(__cle_shVertShadowSmooth, "u_params"); // shadowScattering, shadowAlpha, shadowPenumbra, shadowUmbra
							static _u_shadowSmoothParams2 = shader_get_uniform(__cle_shVertShadowSmooth, "u_params2"); // 2d light position, shadowDepthOffset, lightPenetration
							
							static _u_dirShadowBasicParams = shader_get_uniform(__cle_shVertDirShadowBasic, "u_params"); // shadowScattering, shadowAlpha
							static _u_dirShadowBasicParams2 = shader_get_uniform(__cle_shVertDirShadowBasic, "u_params2"); // xDir, yDir, shadowDepthOffset
							static _u_dirShadowSmoothParams = shader_get_uniform(__cle_shVertDirShadowSmooth, "u_params"); // shadowScattering, shadowAlpha, shadowPenumbra, shadowUmbra
							static _u_dirShadowSmoothParams2 = shader_get_uniform(__cle_shVertDirShadowSmooth, "u_params2"); // xDir, yDir, shadowDepthOffset, lightPenetration
							
							static _u_shadowSpriteParams = shader_get_uniform(__cle_shSpriteShadow, "u_params"); // shadowScattering, shadowAlpha
							static _u_shadowSpriteParams2 = shader_get_uniform(__cle_shSpriteShadow, "u_params2"); // 2d light position, shadowDepthOffset
						...
						#region >> Directional/Sun Lights
							...
							with(__cle_objDirectLight) {
								...
								// shadow
								if (castShadows && _isShadowsEnabled) {
									gpu_set_blendmode_ext_sepalpha(_shadowsBlendMode);
									gpu_set_zfunc(shadowLitType);
									gpu_set_blendequation_sepalpha(bm_eq_add, bm_eq_max); // improves overlap
									if (shadowShaderType & 1) {
										...
										shader_set_uniform_f(_u_dirShadowBasicParams, shadowScattering, shadowAlpha);
										shader_set_uniform_f(_u_dirShadowBasicParams2, dcos(angle), -dsin(angle), shadowDepthOffset);
										vertex_submit(_vBuffVertexShadowsStatic,  pr_trianglelist, -1);
										vertex_submit(_vBuffVertexShadowsDynamic, pr_trianglelist, -1);
									} else
									if (shadowShaderType & 2) {
										gpu_set_cullmode(selfShadows ? cull_noculling : cull_counterclockwise);
										shader_set(__cle_shVertDirShadowSmooth);
										shader_set_uniform_f(_u_dirShadowSmoothParams, shadowScattering, shadowAlpha, shadowPenumbra/100, shadowUmbra);
										shader_set_uniform_f(_u_dirShadowSmoothParams2, dcos(angle), -dsin(angle), shadowDepthOffset, max(penetration*10, CLE_CFG_EPSILON));
										vertex_submit(_vBuffVertexShadowsStatic,  pr_trianglelist, -1);
										vertex_submit(_vBuffVertexShadowsDynamic, pr_trianglelist, -1);
									}
									if (shadowShaderType & 4) {
										gpu_set_cullmode(cull_noculling);
										shader_set(__cle_shDirSpriteShadow);
										shader_set_uniform_f(_u_shadowSpriteParams, shadowScattering, shadowAlpha);
										shader_set_uniform_f(_u_shadowSpriteParams2, dcos(angle), -dsin(angle), shadowDepthOffset);
										__submitSpriteShadows(_vBuffSpriteShadowsStaticBatches, _vBuffSpriteShadowsDynamicBatches);
									}
									gpu_set_cullmode(cull_counterclockwise);
							...
							with(__cle_objShapeLight) {
								...
								if (castShadows && _isShadowsEnabled) {
									...
									if (shadowShaderType & 1) {
										...
										shader_set_uniform_f(_u_shadowBasicParams, shadowScattering, shadowAlpha);
										shader_set_uniform_f(_u_shadowBasicParams2, x, y, shadowDepthOffset);
										vertex_submit(_vBuffVertexShadowsStatic,  pr_trianglelist, -1);
										vertex_submit(_vBuffVertexShadowsDynamic, pr_trianglelist, -1);
									} else 
									if (shadowShaderType & 2) {
										...
										shader_set_uniform_f(_u_shadowSmoothParams, shadowScattering, shadowAlpha, shadowPenumbra, shadowUmbra);
										shader_set_uniform_f(_u_shadowSmoothParams2, x, y, shadowDepthOffset, max(penetration, CLE_CFG_EPSILON));
										vertex_submit(_vBuffVertexShadowsStatic,  pr_trianglelist, -1);
										vertex_submit(_vBuffVertexShadowsDynamic, pr_trianglelist, -1);
									}
									if (shadowShaderType & 4) {
										...
										shader_set_uniform_f(_u_shadowSpriteParams, shadowScattering, shadowAlpha);
										shader_set_uniform_f(_u_shadowSpriteParams2, x, y, shadowDepthOffset);
										__submitSpriteShadows(_vBuffSpriteShadowsStaticBatches, _vBuffSpriteShadowsDynamicBatches);
									}
									gpu_set_cullmode(cull_counterclockwise);
									gpu_set_blendequation(bm_eq_add);
						...
						#region >> Spot Lights
							#region uniforms + shaders
								...
								static _u_spotLightPhong_params4 = shader_get_uniform(__cle_shSpotLightPhong, "u_params4"); // spotFOV, spotSmoothness, spotDistance, cookieAlpha
								...
								static _u_spotLightBRDF_params4 = shader_get_uniform(__cle_shSpotLightBRDF, "u_params4"); // spotFOV, spotSmoothness, spotDistance, cookieAlpha
							...
							with(__cle_objSpotLight) {
								...
								if (shaderType == LIGHT_SHADER_PHONG) {
									...
									shader_set_uniform_f(_u_spotLightPhong_params4, degtorad(spotFOV), max(spotSmoothness, CLE_CFG_EPSILON), spotDistance, clamp(cookieAlpha, 0, 1));
									...
									shader_set_uniform_f(_u_spotLightBRDF_params4, degtorad(spotFOV), max(spotSmoothness, CLE_CFG_EPSILON), spotDistance, clamp(cookieAlpha, 0, 1));
					...
					if (__isGeneratingLightsCollision) {
						__viewProjMatrix = matrix_multiply(camera_get_view_mat(__sourceCamera), camera_get_proj_mat(__sourceCamera));
			...
			#region Combine: Draw surfaces to a final surface
				#region uniforms
					...
					static __uniRenderAmbientColorIsOverlay = shader_get_uniform(__cle_shDeferredRender, "u_ambientColorIsOverlay");
				...
				surface_set_target(_pass.surface);
					...
					if (__deferredFunction == undefined) {
						shader_set(__cle_shDeferredRender);
							...
							shader_set_uniform_f(__uniRenderAmbientColorIsOverlay, __ambientLightColorIsOverlay);
			...
			// ==============================================
			__finalRenderSurf = __renderPass[CRYSTAL_PASS.COMBINE].surface; // final surface is the combine surface
			__cpuFrameTime = get_timer() - _currentFrameTime;
		} else {
			__cpuFrameTime = 0;
			__finalRenderSurf = _surface; // final surface becomes input surface
		}
		return __finalRenderSurf;
	}
	#endregion
	...
	#region DRAW
	/// @desc Draw (if enabled) the final surface on the screen. You can call this in any Draw event, but generally Post-Draw.
	/// This function works like draw_surface_stretched().
	/// NOTE: If you're using post-processing, you MUST not use this function and only Render instead.
	...
	/// @desc Easily draw (if enabled) Lighting renderer's final surface in full screen. It is an easy alternative to the normal .Draw() method.
	///
