...
uniform vec4 u_params4; // spotFOV, spotSmoothness, spotDistance, cookieAlpha
...
void main() {
	...
	vec3 L = normalize(-vec3(u_params3.xy, u_params3.z*u_params5.x));
	...
	if (halfFov < M_PI) {
		...
		if (u_params4.w > 0.0) {
			...
			vec4 cookie = texture2D(u_cookieTexture, mix(u_cookieAtlasUVrect.xy, u_cookieAtlasUVrect.zw, cookieUV));
			lightCookie = mix(lightCookie, cookie.rgb*cookie.a, u_params4.w);
		}
	}
	...
	// Diffuse
	//vec3 diffuse = u_params2.x * lightAttenuation * ((v_vColour.rgb*lightCookie*u_params5.y) + ((kDiffuse*(albedo/M_PI)+specularCT)*v_vColour.rgb*u_params5.z));
	vec3 diffuse = u_params2.x * lightAttenuation * ((v_vColour.rgb*lightCookie*u_params5.y) + (specularCT*v_vColour.rgb*u_params5.z)); // looks better..
